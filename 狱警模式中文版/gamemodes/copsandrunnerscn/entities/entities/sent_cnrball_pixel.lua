...
local BounceSoundPixel = Sound( "copsandrunners/cnrball_pixel.wav" )
...
function ENT:Initialize()
	...
	if ( SERVER ) then
		...
		util.AddNetworkString( "bSoundPixel" )
...
local matBall = Material( "copsandrunners/sent_cnrball_pixel" )
...
function ENT:PhysicsCollide( data, physobj )
	...
	if ( data.Speed > 60 && data.DeltaTime > 0.2 ) then
		...
		--self:EmitSound( BounceSoundPixel, 75, math.random( pitch - 10, pitch + 10 ) )
		net.Start("bSoundPixel")
...
if CLIENT then
	function ballEmitSound(ent)
		...
		--print("hey")
		if IsValid(bBall) then bBall:EmitSound( BounceSoundPixel, 75, math.random( pitch - 10, pitch + 10) ) end
	end

		net.Receive("bSoundPixel", ballEmitSound)
end
