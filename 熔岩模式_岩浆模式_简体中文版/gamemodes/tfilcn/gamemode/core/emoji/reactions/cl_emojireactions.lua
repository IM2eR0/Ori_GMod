hook.RunOnce("HUDPaint", function()
	...
	file.CreateDir("tfilcn")

	if not file.Exists("tfilcn/emojidictionary.txt", "DATA") then
		file.Write("tfilcn/emojidictionary.txt", "[]")
	end

	local EmojiDictionary = file.Read("tfilcn/emojidictionary.txt"):JSONDecode()
		...
		for _, Player in pairs( player.GetAll() ) do
			...
			--if Player == LocalPlayer() then continue end
				...
				cam.Wrap3D2D( function()
					...
					draw.WebImage( "https://s1.ax1x.com/2023/02/14/pSTJiOH.png", 0, 0, 50, 50, pColor() )
