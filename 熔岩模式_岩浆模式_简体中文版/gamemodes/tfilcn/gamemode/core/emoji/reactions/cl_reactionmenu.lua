...
if not file.Exists("tfilcn/emojireactions.txt", "DATA") then
	file.Write("tfilcn/emojireactions.txt", util.TableToJSON(Defaults))
end

local Emotes = file.Read("tfilcn/emojireactions.txt"):JSONDecode()
local ValueTable = {}
		...
		local function RepopulateBar()
			...
			for Emote in Values(Emotes) do
				...
				d.DoClick = function()
					...
					file.Write("tfilcn/emojireactions.txt", util.TableToJSON(Emotes))
					UpdateValueTable()
					ls.Repopulate()
					RepopulateBar()
				end
	...
	for i = 0, (Ratio):floor() - 1 do
		...
		t.Paint = function(s, w, h)
			...
			if s.Hovered then
				...
				s:Declip(function()
					draw.SimpleText( "按住G并点击 来进行设置", "ChatFont", x, y - ScrH()/15, White:Alpha( 50 ), 0 )
				end)
			end
		end
