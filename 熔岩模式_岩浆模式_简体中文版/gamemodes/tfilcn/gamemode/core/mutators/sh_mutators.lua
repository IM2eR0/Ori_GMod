...
function Mutators.StartEvent( event )
	...
	if SERVER then
		...
		Notification.SendType( "Mutator", "准备好了吗？ " .. event .. " 特殊回合要来啦！" )
...
if SERVER then
	hook.Add("Lava.RoundStart", "ShouldStartaMutator", function()
		FrameDelay( function()
			...
			Notification.SendType( "Chance", "让我看看要不要开启特殊回合呢？抽点数吧~！" )
			timer.Simple( 3, function()
				local B = math.random( 1, 6 )
				if A == B then
					Notification.SendType( "Chance", ("抽到了 ${1} 与 ${2}~！ 嗯，看起来不错，让我挑一个特殊回合！"):fill( A, B ))
					timer.Simple( 2, function()
						local Table, Key = table.Random( Mutators.Events )
						Mutators.StartEvent( Key )
					end)
				else
					Notification.SendType( "Chance", ("抽到了 ${1} 与 ${2}~！ 真是可惜呢，这局没选到特殊回合."):fill( A, B ))
				end
			end)
		end)
	end)
