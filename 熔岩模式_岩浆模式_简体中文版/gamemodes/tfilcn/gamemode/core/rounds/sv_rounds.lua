...
function Rounds.Start()
	...
	Notification.Create("回合开始! 熔岩溢流而出!", { SOUND = "ambient/water/drip2.wav", TIME = 5, ICON = "1f31f" })
end
	...
	if Rounds.CurrentState == "Started" then
		...
		if #player.GetAlive() == 1 then
			if #player.GetActive() == 1 then
				...
				ShouldRestart = true
				Notification.SendType( "Winner", player.GetAlive()[1]:Nick() .. " 胜出了!")
...
hook.Add("PostPlayerDeath", "CheckAllDead", function()
	if Rounds.CurrentState == "Started" then
		if #player.GetActive() == 1 then
			Notification.SendType( "Winner", player.GetAll()[1]:Nick() .. " 默认获得胜利!")
		end
