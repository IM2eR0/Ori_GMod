...
hook.RunOnce("HUDPaint", function()
	...
	s.Paint = function(s, w, h)
		...
		draw.SimpleText("<<< 点击小火山可以打开服务器组链接", "lava_score_title_sub", h / 7 * 0.9, h / 14 * 0.6, nil, nil, TEXT_ALIGN_CENTER)
	end
	...
	b.Paint = function( s, w, h )
		...
		draw.SimpleText( ((Config.GetMapSwitchTime() - GetGlobalInt("$NextMapTime", 0 ) ):max( 0 ) .. " 局后进行滚动投票. "), "lava_score_title_sub", w/2, h/2, nil, 1, 1 )
	end
	...
	as.DoClick = function( s )
		gui.OpenURL("https://steamcommunity.com/groups/teasofficial")
	end
		...
		local function AddPlayer(Player, Section )
			...
			p.Paint = function(s, w, h)
				...
				if Player:IsLavaCreator() then
					draw.SimpleText( "熔岩肆溢原作者!", "lava_score_player_row", w/2, h/2, color_white, 1, 1 )
				end

                if Player:SteamID64() == "76561198154133184" then
					draw.SimpleText( "熔岩肆溢MLG玩家!", "lava_score_player_row", w/2, h/2, pColor() + 50 + CurTime():sin() * 100, 1, 1 )
				end
...
hook.Add("Lava.PopulateScoreboardPlayerButtons", "AddDefaultButtons", function( func )
	func( "1f31a", "查看个人资料", function( Player )
		Player:ShowProfile()
	end)
	func( "1f92b", "禁言管理", function( Player, Panel )
		Player:SetMuted( not Player:IsMuted() )
		chat.AddText( ("玩家 ${1} 目前的状态是 ${2}."):fill( Player:Nick(), Player:IsMuted() and "被禁言" or "未禁言") )
	end)
	func( "1f4ac", "切换聊天禁言", function( Player )
		Player.m_IsChatMuted = not Player.m_IsChatMuted
		chat.AddText( ("玩家 ${1} 目前的状态是 聊天框 ${2} (但消息仍会在控制台中显示)."):fill( Player:Nick(), Player.m_IsChatMuted and "被禁言" or "未禁言") )
	end)
	func( "1f4cb", "复制Steam ID", function( Player )
		SetClipboardText( Player:SteamID64() )
		chat.AddText( ("成功复制 ${1} 的Steam ID ${2} ."):fill( Player:Nick(), Player:SteamID64()) )
	end)
