...
AddDatapoint( "排名", "", function( str ) end )
AddDatapoint( "名称", "name", function( str ) return str end, 2 )
AddDatapoint( "常用能力", "ability", function( str ) return str == "NULL" and "N/A" or str end, 2 )
AddDatapoint( "击杀", "kills", Zerofy )
AddDatapoint( "死亡", "deaths", Zerofy )
AddDatapoint( "胜利", "wins", Zerofy )
AddDatapoint( "缺乏", "loses", Zerofy )
AddDatapoint( "鸡蛋投掷量", "eggsthrown", Zerofy )
AddDatapoint( "鸡蛋命中量", "eggshit", Zerofy )
...
hook.Add("Lava.PopulateWidgetMenu", "MakeStatsWindow", function( Context )
	Context.NewWidget( "服务器排行榜", "1f3c6", function()
		Ranking.MakeRequest(page, 25, sel, asc, "", function(data, dataS)
			tbl = {
				data = data,
				dataS = dataS,
